...
function displayProducts() {
    ...
    if (!products || products.length === 0) {
        html = '<tr><td colspan="7" style="text-align: center; padding: 40px;">Chưa có sản phẩm nào</td></tr>';
    } else {
...
/* ================================================= */
/* ============ ẨN/HIỆN/XÓA SẢN PHẨM ============== */
/* ================================================= */

// ✅ Copy toàn bộ đoạn này thay thế 2 hàm cũ
...
function toggleProductStatus(index) {
    ...
    if (confirm(`⚠️ Bạn có chắc muốn ${action} sản phẩm này?\n\n${product.id} - ${product.name}`)) {
        const scrollY = window.scrollY; // Lưu vị trí
        
        products[index].status = product.status === 'active' ? 'hidden' : 'active';
        saveToLocalStorage();
        displayProducts();
        
        setTimeout(() => window.scrollTo(0, scrollY), 0); // Khôi phục
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Preview ảnh khi nhập URL (Modal Thêm)
...
/* ================================================= */
/* ====== TỰ ĐỘNG CẬP NHẬT KHI LOẠI SÁCH THAY ĐỔI === */
/* ================================================= */
